...
export default function AuthPage() {
  ...
  // Handle Google Login (Popup Method - Smoother)
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!docSnap.exists()) {
        // Create new user doc if first time login
        await setDoc(docRef, {
  ...
  return (
    <div className="min-h-screen w-full overflow-hidden bg-gradient-to-br from-gray-50 to-white flex flex-col font-sans">
      ...
      <header className="flex items-center justify-between px-6 md:px-16 h-20 border-b border-gray-200 bg-white">
        ...
        <span className="hidden md:block text-sm font-semibold text-slate-500">
          Made with ❤️ by Gaurav
        </span>
      </header>

      {/* MAIN AREA */}
      <main className="flex-grow flex items-center justify-center px-4 md:px-6 py-10 lg:py-0 overflow-hidden">
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="hidden lg:flex flex-col justify-center space-y-6 pr-6">
            ...
            <div className="space-y-4 mt-4">
                <FeatureItem 
                icon={<FaPenFancy className="text-indigo-600" />}
                title="Write Freely" 
                desc="Express anything without limits"
                />
                <FeatureItem 
                icon={<FaGlobeAmericas className="text-emerald-600" />}
                title="Share Globally" 
                desc="Connect with a worldwide community"
                />
                <FeatureItem 
                icon={<FaShieldAlt className="text-amber-600" />}
                title="Secret Diary" 
                desc="Keep your personal thoughts private"
                />
            </div>
          </div>
          ...
          <div className="w-full max-w-md mx-auto bg-white border border-gray-200 p-8 rounded-3xl shadow-xl">
            ...
            <form onSubmit={handleAuth} className="mt-6 space-y-4">
              ...
              <input
                ...
                className="w-full px-5 py-3 bg-gray-50 border-2 border-gray-200 rounded-2xl text-slate-800 font-semibold outline-none focus:border-slate-900 focus:bg-white transition-all"
              />
